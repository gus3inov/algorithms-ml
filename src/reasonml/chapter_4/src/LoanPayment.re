...
let getStringValue = (id: string) : option(string) => {
    let element = Document.getElementById(id, D.document);
    element
...
let getIntValue = (id: string): option(int) => {
    getStringValue(id) -> Belt.Option.flatMap(toInt)
};

let getFloatValue = (id: string): option(float) => {
    getStringValue(id) -> Belt.Option.flatMap(toFloat)
};
...
let calculate = (_: Dom.event): unit => {
    ...
    let payment = switch(principalValue, aprValue, yearsValue) {
    | (Some(principal), Some(apr), (Some(years))) =>
        Some(Shirts.Payment.payment(
            ~principal = principal,
            ~apr = apr,
            ~years = years
        ));
...
let addEvent = (input: option('a), f: (Dom.event) => unit): unit => switch (input) {
    | Some(element) => 
        Webapi.Dom.EventTarget.addEventListener(
            "input", f, Elem.asEventTarget(element))
    | None => ()
};
